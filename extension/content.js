...
(() => {
  ...
  function onClick(ev) {
    if (!recording) return;
    // If we already handled this click via mouseup logic (either as click or drag), skip
    if (Date.now() < suppressClickUntil || Date.now() - lastClickHandledAt < 200) return;
  ...
  function onMouseUp(ev) {
    ...
    if (dist2 > 25) {
      ...
      const step = {
        type: 'drag',
        selectors: selectorsFor(startEl),
        from: { x: Math.round(dragStart.clientX - (startEl.getBoundingClientRect().left || 0)), y: Math.round(dragStart.clientY - (startEl.getBoundingClientRect().top || 0)) },
        to: { x: ev.clientX, y: ev.clientY },
        path: dragStart.path && dragStart.path.length ? dragStart.path.slice() : undefined,
        targetSelectors: selectorsFor(ev.target),
  ...
  function onClickMaybeSuppress(ev) {
    if (!recording) return;
    if (Date.now() < suppressClickUntil) {
      // skip click generated by drag
      ev.stopPropagation?.();
      return;
    }
  }
  ...
  function start(startedAt, isInitial) {
    ...
    window.addEventListener('click', onClickMaybeSuppress, true);
  ...
  function stop() {
    ...
    window.removeEventListener('click', onClickMaybeSuppress, true);
